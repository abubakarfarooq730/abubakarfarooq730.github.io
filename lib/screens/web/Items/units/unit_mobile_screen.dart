...
import 'package:sellio/models/unit_mode.dart';
import 'package:syncfusion_flutter_datagrid/datagrid.dart';

import '../../../constants/create_new_button.dart';
import '../../../utils/custom_textform_field_withicon.dart';

class UnitMobileScreen extends StatefulWidget {
  const UnitMobileScreen({super.key});

  @override
  State<UnitMobileScreen> createState() => _UnitMobileScreenState();
}

class _UnitMobileScreenState extends State<UnitMobileScreen> {
...
class ItemDataSource extends DataGridSource {
  ...
  ItemDataSource(List<Unit> items)
      : _dataGridRows = items
      .map(
        (item) => DataGridRow(
      cells: [
        DataGridCell<String>(columnName: 'Title', value: item.title),
        DataGridCell<String>(columnName: 'Short Title', value: item.shortTitle),
        DataGridCell<int>(columnName: 'Number of Products', value: item.count),
      ],
    ),
  )
      .toList();
...
Widget _buildPaginationControls() {
  return Row(
    children: [
      IconButton(
        icon: Icon(Icons.chevron_left, color: AppColors.buttonColor,),
        onPressed: () {},
      ),
      const Text("1 of 2", style: TextStyle(
        fontSize: 16,
        fontWeight: FontWeight.bold,
        color: Colors.black,
      ),),
      IconButton(
        icon: Icon(Icons.chevron_right, color: AppColors.buttonColor,),
        onPressed: () {},
      ),
    ],
  );
