...
class PurchaseReportWebScreen extends StatefulWidget {
  const PurchaseReportWebScreen({super.key});

  @override
  State<PurchaseReportWebScreen> createState() =>
      _PurchaseReportWebScreenState();
}

class _PurchaseReportWebScreenState extends State<PurchaseReportWebScreen> {
  ...
  void initState() {
    super.initState();
    _purchaseReportDataSource = PurchaseReportDataSource(
  ...
  Future<void> generateAndDownloadPDF() async {
    ...
    page.graphics.drawString(
      'Total Purchase: \$${totalBalance.toStringAsFixed(2)}',
      PdfStandardFont(PdfFontFamily.helvetica, 12),
    ...
    for (var report in getCustomerReportData()) {
      ...
      row.cells[0].value = report.purchaseNumber.toString();
      row.cells[1].value = report.date;;
      row.cells[2].value = report.supplier;
      row.cells[3].value = report.items;
      row.cells[4].value = report.subtotal.toString();
      row.cells[5].value = report.discount.toString();
      row.cells[6].value = report.tax.toString();
          ...
          children: [
            ...
            Row(
              ...
              children: [
                ...
                _buildStatCard('Total Pay', '15000', Colors.green),
...
class PurchaseReportDataSource extends DataGridSource {
  final List<PurchaseReport> reports;
