...
class SalesReportWebScreen extends StatefulWidget {
  const SalesReportWebScreen({super.key});

  @override
  State<SalesReportWebScreen> createState() =>
      _SalesReportWebScreenState();
}

class _SalesReportWebScreenState extends State<SalesReportWebScreen> {
  late CustomerReportDataSource _salesReportDataSource;
  final Set<int> _highlightedRows = {};

  @override
  void initState() {
    super.initState();
    _salesReportDataSource = CustomerReportDataSource(
  ...
  List<SalesReport> getCustomerReportData() {
    return [
      SalesReport(
          1, 'Naeem',  5000, 500, 200, 100, 5400, 3000),
      SalesReport(
          2, 'Rasheed',  4000, 400, 150, 80, 4230, 2500),
      SalesReport(
          3, 'Moiz',  3000, 300, 100, 50, 3150, 2000, ),
      SalesReport(
          4, 'Sabir',2000, 200, 50, 20, 2070, 1000, ),
    ];
  }
  ...
  Future<void> generateAndDownloadPDF() async {
    ...
    double totalBalance = getCustomerReportData()
        .fold(0, (sum, report) => sum + report.subtotal);
    double totalPay =
    ...
    page.graphics.drawString(
      'Total Sales: \$${totalBalance.toStringAsFixed(2)}',
      PdfStandardFont(PdfFontFamily.helvetica, 12),
      bounds: const Rect.fromLTWH(0, 80, 500, 30),
    );
    page.graphics.drawString(
      'Total Profit: \$${totalRemaining.toStringAsFixed(2)}',
      PdfStandardFont(PdfFontFamily.helvetica, 12),
    ...
    for (var report in getCustomerReportData()) {
      ...
      row.cells[1].value = report.customer;
      row.cells[2].value = report.subtotal.toString();
      row.cells[3].value = report.discount.toString();
      row.cells[4].value = report.tax.toString();
      row.cells[5].value = report.extraAmount.toString();
      row.cells[6].value = report.grandTotal.toString();
      row.cells[7].value = report.profit.toString();
    }
          ...
          children: [
            ...
            Row(
              ...
              children: [
                ...
                _buildStatCard('Total Sales', '20000', Colors.orange),
                _buildStatCard('Total Profit', '15000', Colors.green),
              ],
            ),
  ...
  CustomerReportDataSource(
    ...
    _dataGridRows = reports.map((report) {
      return DataGridRow(cells: [
        ...
        DataGridCell<double>(columnName: 'paid', value: report.profit),
      ]);
    }).toList();
  }
